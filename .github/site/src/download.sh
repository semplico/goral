...
main() {
    ...
    local _unpacked_dir="${_dir}/${_file}"
    local _output_file="${_unpacked_dir}.tar.gz"
...
get_architecture() {
    ...
    case "$_ostype" in
        ...
        *)
            err "unsupported OS type: $_ostype, try building from source"
            ;;
    ...
    case "$_cputype" in
        ...
        *)
            err "unsupported CPU type: $_cputype, try building from source"

    esac

    # Detect 64-bit linux with 32-bit userland
    if [ "${_ostype}" = unknown-linux-gnu ] && [ "${_bitness}" -eq 32 ]; then
        err "linux with 32-bit userland is unsupported, try building from source"
    fi
